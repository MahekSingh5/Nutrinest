...
const Products = () => {
  ...
  // Try to fetch real products from backend and use them if available
  useEffect(() => {
    let mounted = true;
    const fetchProducts = async () => {
      try {
        ...
        if (data && Array.isArray(data.products) && data.products.length) {
          // normalize product objects to the admin local shape
  ...
  const handleDelete = (p) => {
    setConfirm({ open: true, product: p });
  };
  ...
  return (
    <div className="container-fluid">
      ...
      <ConfirmModal
        ...
        message={`Delete ${confirm.product?.name}?`}
